...
تأكد من توفر Node.js `22.22.0` أو إصدار Node 22 متوافق مع Vite 8، ثم قم بتثبيت الاعتماديات:
...
The frontend CI pipeline runs on GitHub Actions with Node.js `22.22.0` and npm.
...
`npm run verify` runs the quality gates in this order:
...
Pull requests targeting `dev` or `main` run the frontend CI workflow. Pushes to
`dev`, `main`, and the temporary `chore/frontend-ci-cd` branch also run the same
checks. The workflow has read-only repository permissions (`contents: read`) and
does not request `contents: write` or `pull-requests: write`.

Coverage is generated by Vitest with the V8 provider and uploaded as a workflow
artifact. The production build remains `tsc -b && vite build`, and the output is
uploaded from `dist/`.
...
Do not append `/api/v1`; frontend endpoint constants already include the
versioned API prefix. Frontend CI must never receive backend secrets such as
`DATABASE_URL`, JWT secrets, Supabase service role keys, or payment webhook
secrets.

Vercel remains the deployment target through `vercel.json`. If the Vercel GitHub
integration is connected, GitHub Actions validates the branch/PR and Vercel
handles preview/staging/production deployment after the relevant branch updates.
No duplicate deployment workflow is added here.

CI/CD never merges pull requests. Merge remains a manual administrator action.
